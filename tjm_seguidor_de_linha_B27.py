...
while True:
    ...
    #detecao de Verde cruzamentos/intersecoes
        ...
        # --- VELOCIDADE DINÂMICA ---
        # 1. Compensação de bateria (Bateria do EV3 carregada tem aprox. 8000mV)
        # Se a bateria cair, o fator diminui a velocidade máxima proporcionalmente para manter o robô previsível
        fator_bateria = ev3.battery.voltage() / 8000.0
        potencia_max = potencia * fator_bateria
        
        # 2. Compensação de reflexão
        # Como pedido: mais luz (branco) = menos potência. Menos luz (preto) = mais potência.
        # Isso faz o robô acelerar nas retas (bem centralizado) e frear nas curvas (quando começa a ver mais branco)
        reflexao_media = (ref_sensor_esquerda + ref_sensor_direita) / 2
        proporcao_branco = min(max(reflexao_media / 70.0, 0.0), 1.0) # Garante que fique entre 0 e 1
        
        # Quando estiver no branco total (proporcao = 1), reduz a velocidade em 50% (0.5).
        # Você pode alterar esse 0.5 caso queira que ele freie mais (ex: 0.7) ou menos (ex: 0.3)
        potencia_dinamica = potencia_max - (proporcao_branco * (potencia_max * 0.5))
