...
robot = DriveBase(motor_esquerda, motor_direita, wheel_diameter=55, axle_track=104) #ajustar conforme o robô
...
def mostrar(): #Função para testes, mostrando os valores enquanto o código está rodando
...
while True:
    if sensor_ultrasonico.distance() < 120:  # Distância em mm
        caixa()
    else:
        #mostrar()
        if sensor_direita.color() == Color.RED: #and sensor_esquerda.color() != Color.GREEN:
            ...
            erro_anterior = 0
            integral = 0
            kp = 4.2 #oscila muito → diminuir KP; responde lento → aumentar KP; tremendo → aumentar KD; puxando para um lado → ajustar OFFSET; erro contínuo → aumentar KI;
            ki = 0.02
            kd = 1.2
