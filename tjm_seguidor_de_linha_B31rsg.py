...
def ir_ate_vitima(velocidade=40, max_graus_aproximacao=800):
...
def pegar_vitima():
    ...
    motor_garra.stop()
    '''
    motor_garra.reset_angle()
    angulo_total = 200
    if sentido == "abrir":
        while motor_garra.angle() < angulo_total:
            motor_garra.dc(100)
            wait(10)
    elif sentido == "fechar":
        while motor_garra.angle() < angulo_total *(-1):
            motor_garra.dc(-100)
            wait(10)
    motor_garra.stop()
    '''
