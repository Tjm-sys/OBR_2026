...
def mover_compartimento(potencia_motor):
    log_estado("Liberando vitimas")
    robot.stop()
    motor_esquerda.reset_angle(0)
    motor_direita.reset_angle(0)
    ...
    velocidade = abs(potencia_motor) * 9
    #o sinal da potencia determina se o robo anda para frente ou para tras
    angulo = 90 if potencia_motor >= 0 else -90
...
while True:
    ...
    #detecao de Verde cruzamentos/intersecoes
        ...
        #agora que as portas estao arrumadas, verde na direita vira para a direita
            ...
            # Achamos um verde VERDADEIRO na direita.
            # Como o robô pode estar torto, vamos avançar um pouquinho e checar o sensor esquerdo
            # para garantir que não é um Verde Duplo.
            robot.drive(50, 0)
            wait(150)
            robot.stop()
            if sensor_esquerda.color() == Color.GREEN and (4 <= sensor_esquerda.reflection() <= 10):
                log_estado("Verde Duplo 180")
                virar_180_bloco_sgiro(175, 40)
            else:
                log_estado("Verde Direita")
                robot.drive(300, 0) 
                wait(350) # Completa o avanço (era 500, já andamos 150)
                robot.stop()
                virar_bloco_sgiro_verde(90, "direita", 40)

        elif verde_esq:
            # Achamos um verde VERDADEIRO na esquerda.
            # Checa se não tem um verde direito um pouco mais a frente (robô torto).
            robot.drive(50, 0)
            wait(150)
            robot.stop()
            if sensor_direita.color() == Color.GREEN and (4 <= sensor_direita.reflection() <= 10):
                log_estado("Verde Duplo 180")
                virar_180_bloco_sgiro(175, 40)
            else:
